...
class VertexAIClient(LLMAPIClient):
    """Client for calling Claude API via Google Cloud Vertex AI."""
    ...
    def __init__(self, 
        ...
        Args:
            model: Claude model to use (will be converted to Vertex AI format)
        ...
        if not self.project_id:
            raise ValueError(
                "No Google Cloud project ID found. Please set GOOGLE_CLOUD_PROJECT environment variable "
        ...
        try:
            ...
            logger.info(f"Vertex AI client initialized successfully for project {self.project_id} in {self.region}")
        except ImportError:
            raise ImportError(
                "anthropic[vertex] package is required for Vertex AI support. "
    ...
    def _convert_model_name(self, model: str) -> str:
        ...
        Vertex AI uses @ instead of - in version strings.
        Examples:
        - claude-opus-4-20250514 -> claude-opus-4@20250514
        - claude-3-5-sonnet-20240620 -> claude-3-5-sonnet@20240620
        """
        # Replace the last dash before the version with @
        if '-20' in model:
            parts = model.split('-')
            # Find the last part that starts with '20' (year)
            for i in range(len(parts) - 1, -1, -1):
                if parts[i].startswith('20') and len(parts[i]) == 8:
                    # Join everything before this part with '-' and this part with '@'
                    return '-'.join(parts[:i]) + '@' + parts[i]
        return model
    
    def validate_api_access(self) -> Tuple[bool, str]:
        """Validate that Vertex AI API access is working.
        ...
        try:
            ...
            logger.info("Vertex AI API access validated successfully")
            return True, ""
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Vertex AI API validation failed: {error_msg}")
    ...
    def call_with_retry(self, 
        ...
        """Make Vertex AI API call with retry logic.
        ...
        while retries <= self.max_retries:
            try:
                logger.info(f"Vertex AI API call attempt {retries + 1}/{self.max_retries + 1}")
                ...
                logger.info(f"Vertex AI API call successful in {duration:.1f}s")
                return True, response_text, ""
                
            except Exception as e:
                error_msg = str(e)
                last_error = error_msg
                logger.error(f"Vertex AI API call failed: {error_msg}")
    ...
    def analyze_single_finding(self, 
        ...
        """Analyze a single security finding to filter false positives using Vertex AI.
        ...
        try:
            ...
            success, analysis_result = parse_json_with_fallbacks(response_text, "Vertex AI API response")
            if success:
                logger.info("Successfully parsed Vertex AI API response for single finding")
